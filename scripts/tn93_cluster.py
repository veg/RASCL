# =============================================================================
# TN93 Genetic distance clustering
# =============================================================================
...
arguments = argparse.ArgumentParser(
    description='Cluster an MSA with genetic distance (TN93)')
...
arguments.add_argument(
    '--threshold',
    help='Distance threshold for clustering query sequences',
    required=True,
    type=float)
arguments.add_argument(
    '-m',
    '--max_retain',
    help='The maximum number of sequences to retain',
    required=True,
    type=int)
arguments.add_argument(
    '-r',
...
def run_command(_exec, arguments, filename, tag):
    global input_stamp
    cmd = " ".join([_exec] + arguments)
    print(cmd)
...
def cluster_to_fasta(in_file, out_file, ref_seq=None):
    # assert that in_file exists, otherwise this will crash if tn93-cluster
    # did not run.
        ...
        with open(out_file, "w") as fh2:
            for c in cluster_json:
                ...
                while seq_id in check_uniq:
                    seq_id = cc[0] + '_' + \
                        ''.join(random.choices('0123456789abcdef', k=10))
                # end while
...
# =============================================================================
# Output is actually the compressed.fas, thats what we want.
# Input is the MSA.
...
while True:
    input_stamp = run_command(
        task_runners['tn93-cluster'],
        [
            '-f',
            '-o',
            cluster_json,
            ...
            input_file],
        cluster_json,
        "extract representative clusters at threshold %g" %
        threshold)
    if _ref_seq_name != "":
        input_stamp, cluster_count = cluster_to_fasta(
            cluster_json, compressed_fasta, _ref_seq_name)
    else:
        # changes the json to fasta, also returns the count len().
        input_stamp, cluster_count = cluster_to_fasta(
            cluster_json, compressed_fasta)
    # end if
